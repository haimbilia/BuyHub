 ...
 * Documentation : https://developer.safaricom.co.ke/Documentation (LIPA NA M-PESA ONLINE API)
 */
class Mpesa extends PaymentMethodBase
{
    ...
    public function generateToken(): bool
    {
        ...
        if (!$this->response = curl_exec($curl)) {
            ...
            if (empty($this->error)) {
				$this->error = Labels::getLabel('ERR_UNABLE_TO_GENERATE_TOKEN', $this->langId);
			}
            return false;
        }
        curl_close($curl);
