...
class PaystackSettingsController extends PaymentMethodSettingsController
{
    public static function form(int $langId)
    {
        ...
        $fld1 = new FormFieldRequirement('secret_key', Labels::getLabel('FRM_SECRET_KEY', $langId));
        $fld1->setRequired(false);
        $reqFld1 = new FormFieldRequirement('secret_key', Labels::getLabel('FRM_SECRET_KEY', $langId));
        $reqFld1->setRequired(true);

        $frm->addTextBox(Labels::getLabel('FRM_PUBLIC_KEY', $langId), 'public_key');
        $fld2 = new FormFieldRequirement('public_key', Labels::getLabel('FRM_PUBLIC_KEY', $langId));
        $fld2->setRequired(false);
        $reqFld2 = new FormFieldRequirement('public_key', Labels::getLabel('FRM_PUBLIC_KEY', $langId));
        $reqFld2->setRequired(true);


        $frm->addTextBox(Labels::getLabel('FRM_SECRET_KEY', $langId), 'live_secret_key');
        $liveFld1 = new FormFieldRequirement('live_secret_key', Labels::getLabel('FRM_SECRET_KEY', $langId));
        $liveFld1->setRequired(false);
        $reqLiveFld1 = new FormFieldRequirement('live_secret_key', Labels::getLabel('FRM_SECRET_KEY', $langId));
        $reqLiveFld1->setRequired(true);

        $frm->addTextBox(Labels::getLabel('FRM_PUBLIC_KEY', $langId), 'live_public_key');
        $liveFld2 = new FormFieldRequirement('live_public_key', Labels::getLabel('FRM_PUBLIC_KEY', $langId));
        $liveFld2->setRequired(false);
        $reqLiveFld2 = new FormFieldRequirement('live_public_key', Labels::getLabel('FRM_PUBLIC_KEY', $langId));
