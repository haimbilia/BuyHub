...
class PayfastSettingsController extends PaymentMethodSettingsController
{
    public static function form(int $langId)
    {
        ...
        $reqFld1 = new FormFieldRequirement('merchant_id', Labels::getLabel('FRM_MERCHANT_ID', $langId));
        $reqFld1->setRequired(true);

        $frm->addTextBox(Labels::getLabel('FRM_MERCHANT_KEY', $langId), 'merchant_key');
        $fld2 = new FormFieldRequirement('merchant_key', Labels::getLabel('FRM_MERCHANT_KEY', $langId));
        $fld2->setRequired(false);
        $reqFld2 = new FormFieldRequirement('merchant_key', Labels::getLabel('FRM_MERCHANT_KEY', $langId));
        $reqFld2->setRequired(true);


        $frm->addTextBox(Labels::getLabel('FRM_MERCHANT_ID', $langId), 'live_merchant_id');
        $liveFld1 = new FormFieldRequirement('live_merchant_id', Labels::getLabel('FRM_MERCHANT_ID', $langId));
        $liveFld1->setRequired(false);
        $reqLiveFld1 = new FormFieldRequirement('live_merchant_id', Labels::getLabel('FRM_MERCHANT_ID', $langId));
        $reqLiveFld1->setRequired(true);

        $frm->addTextBox(Labels::getLabel('FRM_MERCHANT_KEY', $langId), 'live_merchant_key');
        $liveFld2 = new FormFieldRequirement('live_merchant_key', Labels::getLabel('FRM_MERCHANT_KEY', $langId));
        $liveFld2->setRequired(false);
        $reqLiveFld2 = new FormFieldRequirement('live_merchant_key', Labels::getLabel('FRM_MERCHANT_KEY', $langId));
