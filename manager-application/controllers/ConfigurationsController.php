...
class ConfigurationsController extends AdminBaseController
{
    ...
    public function setGeneralForm($frmType)
    {
    ...
    public function form(int $frmType)
    {
        $this->setGeneralForm($frmType);
        $this->_template->render(false, false);
    }

    public function confLangForm($frmType, $langId, $tabId = null)
    {
    ...
    private function getForm($type, $arrValues = array())
    {
        ...
        switch ($type) {
            case Configurations::FORM_GENERAL:
                $frm->addEmailField(Labels::getLabel('LBL_Store_Owner_Email', $this->siteLangId), 'CONF_SITE_OWNER_EMAIL');
                $frm->addHiddenField('', 'CONF_SITE_PHONE_dcode');
                $phnFld = $frm->addTextBox(Labels::getLabel('LBL_Telephone', $this->siteLangId), 'CONF_SITE_PHONE', '', array('class' => 'phone-js ltr-right', 'placeholder' => ValidateElement::PHONE_NO_FORMAT, 'maxlength' => ValidateElement::PHONE_NO_LENGTH));
                ...
                $faxFld = $frm->addTextBox(Labels::getLabel('LBL_Fax', $this->siteLangId), 'CONF_SITE_FAX', '', array('class' => 'phone-js ltr-right', 'placeholder' => ValidateElement::PHONE_NO_FORMAT, 'maxlength' => ValidateElement::PHONE_NO_LENGTH));
                ...
                $frm->addSelectBox(Labels::getLabel('LBL_About_Us', $this->siteLangId), 'CONF_ABOUT_US_PAGE', $cpagesArr, '', [], Labels::getLabel('LBL_Select', $this->siteLangId));
                $frm->addSelectBox(Labels::getLabel('LBL_Privacy_Policy_Page', $this->siteLangId), 'CONF_PRIVACY_POLICY_PAGE', $cpagesArr, '', [], Labels::getLabel('LBL_Select', $this->siteLangId));
                $frm->addSelectBox(Labels::getLabel('LBL_Terms_and_Conditions_Page', $this->siteLangId), 'CONF_TERMS_AND_CONDITIONS_PAGE', $cpagesArr, '', [], Labels::getLabel('LBL_Select', $this->siteLangId));
                ...
                $fld1->htmlAfterField = "<small>" . Labels::getLabel("LBL_cookies_policies_section_will_be_shown_on_frontend", $this->siteLangId) . "</small>";
            ...
            case Configurations::FORM_LOCAL:
                $frm->addSelectBox(
                    Labels::getLabel('LBL_Default_Site_Laguage', $this->siteLangId),
                ...
                $fld = $frm->addSelectBox(Labels::getLabel('LBL_Timezone', $this->siteLangId), 'CONF_TIMEZONE', Configurations::dateTimeZoneArr(), false, array(), '');
                $fld->htmlAfterField = '<small>' . Labels::getLabel("LBL_Current", $this->siteLangId) . ' <span id="currentDate">' . CommonHelper::currentDateTime(null, true) . '</span></small>';
                $countryObj = new Countries();
                $countriesArr = $countryObj->getCountriesAssocArr($this->siteLangId);
                $fld = $frm->addSelectBox(Labels::getLabel('LBL_Country', $this->siteLangId), 'CONF_COUNTRY', $countriesArr, '', [], Labels::getLabel('LBL_Select', $this->siteLangId));

                $frm->addSelectBox(Labels::getLabel('LBL_State', $this->siteLangId), 'CONF_STATE', array(), '', [], Labels::getLabel('LBL_Select', $this->siteLangId));
                $frm->addRequiredField(Labels::getLabel("LBL_Postal_Code", $this->siteLangId), 'CONF_ZIP_CODE');
                $frm->addSelectBox(Labels::getLabel('LBL_date_Format', $this->siteLangId), 'CONF_DATE_FORMAT', Configurations::dateFormatPhpArr(), false, array(), '');

                $currencyArr = Currency::getCurrencyNameWithCode($this->siteLangId);
                $frm->addSelectBox(Labels::getLabel('LBL_Default_System_Currency', $this->siteLangId), 'CONF_CURRENCY', $currencyArr, false, array(), '');

                $currencySeparatorArr = applicationConstants::currencySeparatorArr($this->siteLangId);
                $frm->addSelectBox(Labels::getLabel('LBL_Default_Currency_Decimal_Separator', $this->siteLangId), 'CONF_DEFAULT_CURRENCY_SEPARATOR', $currencySeparatorArr, false, array(), '');


                $faqCategoriesArr = FaqCategory::getFaqPageCategories();
                $sellerCategoriesArr = FaqCategory::getSellerPageCategories();

                $frm->addSelectBox(Labels::getLabel('LBL_Faq_Page_Main_Category', $this->siteLangId), 'CONF_FAQ_PAGE_MAIN_CATEGORY', $faqCategoriesArr, '', [], Labels::getLabel('LBL_Select', $this->siteLangId));
                $frm->addSelectBox(Labels::getLabel('LBL_Seller_Page_Main_Faq_Category', $this->siteLangId), 'CONF_SELLER_PAGE_MAIN_CATEGORY', $sellerCategoriesArr, '', [], Labels::getLabel('LBL_Select', $this->siteLangId));
            ...
            case Configurations::FORM_SEO:
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("LBL_LANGUAGE_CODE_TO_SITE_URLS_EXAMPLES", $this->siteLangId) . '</small>';

                $fld = $frm->addTextBox(Labels::getLabel('LBL_Twitter_Username', $this->siteLangId), 'CONF_TWITTER_USERNAME');
                $fld->htmlAfterField = '<small>' . Labels::getLabel("LBL_This_is_required_for_Twitter_Card_code_SEO_Update", $this->siteLangId) . '</small>';

                $fld2 = $frm->addTextarea(Labels::getLabel('LBL_Site_Tracker_Code', $this->siteLangId), 'CONF_SITE_TRACKER_CODE');
                $fld2->htmlAfterField = '<small>' . Labels::getLabel("LBL_This_is_the_site_tracker_script,_used_to_track_and_analyze_data_about_how_people_are_getting_to_your_website._e.g.,_Google_Analytics.", $this->siteLangId) . ' http://www.google.com/analytics/</small>';

                $robotsFld = $frm->addTextarea(Labels::getLabel('LBL_Robots_Txt', $this->siteLangId), 'CONF_SITE_ROBOTS_TXT');
                $robotsFld->htmlAfterField = '<small>' . Labels::getLabel("LBL_This_will_update_your_Robots.txt_file._This_is_to_help_search_engines_index_your_site_more_appropriately.", $this->siteLangId) . '</small>';
            ...
            case Configurations::FORM_PRODUCT:
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Shipping_packages_are_required_in_case_Shipping_API_is_enabled", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_On_Enabling_This_Feature,_Admin_Need_To_Approve_the_brand_requests_(User_Cannot_link_the_requested_brand_with_any_product_until_it_gets_approved_by_Admin)", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_On_Enabling_This_Feature,_Admin_Need_To_Approve_the_Product_category_requests_(User_Cannot_link_the_requested_category_with_any_product_until_it_gets_approved_by_Admin)", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_will_enable_tax_categories_code", $this->siteLangId) . "</small>";
                ...
                $fld3 = $frm->addTextBox(Labels::getLabel("LBL_Default_Items_Per_Page_(Catalog)", $this->siteLangId), "CONF_ITEMS_PER_PAGE_CATALOG");
                $fld3->requirements()->setInt();
                $fld3->requirements()->setRange('1', '2000');
                ...
                $fld = $frm->addRadioButtons(
                    Labels::getLabel("LBL_PRODUCT_LISTING", $this->siteLangId),
                    'CONF_PRODUCT_GEO_LOCATION',
                    $prodGeoSettingArr,
                    '',
                    array('class' => 'list-inline')
                );
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_DISPLAY_AND_SEARCH_PRODUCTS_BASED_ON_LOCATION", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_DISPLAY_AND_SEARCH_PRODUCTS_BASED_ON_CRITERIA", $this->siteLangId) . "</small>";

                $fld = $frm->addTextBox(Labels::getLabel('LBL_RADIUS_MAX_DISTANCE_IN_MILES', $this->siteLangId), 'CONF_RADIUS_DISTANCE_IN_MILES');
                $fld->requirements()->setInt();

                $fld = $frm->addRadioButtons(
                    Labels::getLabel("LBL_SET_DEFAULT_GEO_LOCATION", $this->siteLangId),
                    'CONF_DEFAULT_GEO_LOCATION',
                    applicationConstants::getYesNoArr($this->siteLangId),
                    '0',
                    array('class' => 'list-inline')
                );
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_SET_DEFAULT_LOCATION_FOR_PRODUCT_LISTING", $this->siteLangId) . "</small>";

                $countryObj = new Countries();
                $countriesArr = $countryObj->getCountriesAssocArr($this->siteLangId, true, 'country_code');
                $fld = $frm->addSelectBox(Labels::getLabel('LBL_Country', $this->siteLangId), 'CONF_GEO_DEFAULT_COUNTRY', $countriesArr, '', [], Labels::getLabel('LBL_Select', $this->siteLangId));

                $frm->addSelectBox(Labels::getLabel('LBL_State', $this->siteLangId), 'CONF_GEO_DEFAULT_STATE', array(), '', [], Labels::getLabel('LBL_Select', $this->siteLangId));
                $frm->addTextBox(Labels::getLabel("LBL_Postal_Code", $this->siteLangId), 'CONF_GEO_DEFAULT_ZIPCODE');
            ...
            case Configurations::FORM_USER_ACCOUNT:
                ...
                $fld5->htmlAfterField = '<small>' . Labels::getLabel("LBL_On_enabling_this_feature,_admin_need_to_approve_each_user_after_registration_(User_cannot_login_until_admin_approves)", $this->siteLangId) . '</small>';
                ...
                $fld7->htmlAfterField = "<small>" . Labels::getLabel("LBL_user_need_to_verify_their_email_address_provided_during_registration", $this->siteLangId) . " </small>";
                ...
                $fld8->htmlAfterField = "<small>" . Labels::getLabel("LBL_On_enabling_this_feature,_notification_mail_will_be_sent_to_administrator_on_each_registration.", $this->siteLangId) . "</small>";
                ...
                $fld9->htmlAfterField = "<small>" . Labels::getLabel("LBL_On_enabling_this_feature,_users_will_be_automatically_logged-in_after_registration", $this->siteLangId) . "</small>";
                ...
                $fld10->htmlAfterField = "<small>" . Labels::getLabel("LBL_On_enabling_this_feature,_users_will_receive_a_welcome_mail_after_registration.", $this->siteLangId) . "</small>";
                ...
                $fld11->htmlAfterField = "<small>" . Labels::getLabel("LBL_On_enabling_this_feature,_buyers_and_seller_will_have_a_separate_sign_up_form.", $this->siteLangId) . "</small>";
                ...
                $fld6->htmlAfterField = "<small>" . Labels::getLabel("LBL_On_enabling_this_feature,_admin_need_to_approve_Seller's_request_after_registration", $this->siteLangId) . "</small>";
                ...
                $fld11->htmlAfterField = "<small>" . Labels::getLabel("LBL_On_enabling_this_feature,_buyers_will_be_able_to_see_Seller_tab", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Maximum_seller_request_attempts_allowed", $this->siteLangId) . "</small>";

                $frm->addHtml('', 'Withdrawal', '<div class="separator separator-dashed my-2"></div><h3 class="form-section-head">' . Labels::getLabel("LBL_Withdrawal", $this->siteLangId) . '</h3>');

                $fld = $frm->addIntegerField(Labels::getLabel("LBL_Minimum_Withdrawal_Amount", $this->siteLangId) . ' [' . $this->siteDefaultCurrencyCode . ']', 'CONF_MIN_WITHDRAW_LIMIT', '');
                $fld->htmlAfterField = "<small> " . Labels::getLabel("LBL_This_is_the_minimum_withdrawable_amount.", $this->siteLangId) . "</small>";

                $fld = $frm->addIntegerField(Labels::getLabel("LBL_Maximum_Withdrawal_Amount", $this->siteLangId) . ' [' . $this->siteDefaultCurrencyCode . ']', 'CONF_MAX_WITHDRAW_LIMIT', '');
                $fld->htmlAfterField = "<small> " . Labels::getLabel("LBL_This_is_the_maximum_withdrawable_amount.", $this->siteLangId) . "</small>";

                $fld = $frm->addIntegerField(Labels::getLabel("LBL_Minimum_Interval_[Days]", $this->siteLangId), 'CONF_MIN_INTERVAL_WITHDRAW_REQUESTS', '');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_minimum_interval_in_days_between_two_withdrawal_requests.", $this->siteLangId) . "</small>";
                break;

            case Configurations::FORM_CHECKOUT_PROCESS:
                $frm->addHtml('', 'Checkout', '<h3 class="form-section-head">' . Labels::getLabel('LBL_COD_Payments', $this->siteLangId) . '</h3>');
                $fld = $frm->addTextBox(Labels::getLabel('LBL_Minimum_COD_Order_Total', $this->siteLangId) . ' [' . $this->siteDefaultCurrencyCode . ']', 'CONF_MIN_COD_ORDER_LIMIT');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_minimum_cash_on_delivery_order_total,_eligible_for_COD_payments.", $this->siteLangId) . "</small>";
                $fld = $frm->addTextBox(Labels::getLabel('LBL_Maximum_COD_Order_Total', $this->siteLangId) . ' [' . $this->siteDefaultCurrencyCode . ']', 'CONF_MAX_COD_ORDER_LIMIT');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_maximum_cash_on_delivery_order_total,_eligible_for_COD_payments._Default_is_0", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_seller_needs_to_maintain_to_accept_COD_orders._Default_is_-1", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_SHOP_PICKUP_INTERVAL_INFO", $this->siteLangId) . "</small>";
                ...
                $fld1->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_Transaction_Mode_to_live_environment", $this->siteLangId) . "</small>";
                $obj = new Plugin();
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("LBL_On_enabling_this_feature,_seller_will_be_able_to_collect_tax", $this->siteLangId) . '</small>';

                $fld = $frm->addCheckBox(Labels::getLabel("LBL_TAX_AFTER_DISCOUNTS", $this->siteLangId), 'CONF_TAX_AFTER_DISOCUNT', 1, array(), false, 0);
                $fld->htmlAfterField = '<small>' . Labels::getLabel("LBL_On_enabling_this_feature,_tax_will_be_applicable_after_discounts", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("LBL_On_enabling_return_shipping_charges_to_customer,", $this->siteLangId) . '</small>';

                $fld = $frm->addCheckBox(Labels::getLabel("LBL_SHIPPED_BY_ADMIN_ONLY", $this->siteLangId), 'CONF_SHIPPED_BY_ADMIN_ONLY', 1, array(), false, 0);
                $fld->htmlAfterField = '<small>' . Labels::getLabel("LBL_On_enabling_shipping_charges_manged_by_admin_only,", $this->siteLangId) . '</small>';
                ...
                $fld = $frm->addSelectBox(
                    Labels::getLabel("LBL_Default_Paid_Order_Status", $this->siteLangId),
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_default_child_order_status_when_an_order_is_marked_Paid.", $this->siteLangId) . "</small>";

                $fld = $frm->addSelectBox(
                    Labels::getLabel("LBL_DEFAULT_APPROVED_ORDER_STATUS", $this->siteLangId),
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_SET_THE_DEFAULT_APPROVED_ORDER_STATUS", $this->siteLangId) . "</small>";

                $fld = $frm->addSelectBox(
                    Labels::getLabel("LBL_Default_InProcess_Order_Status", $this->siteLangId),
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_default_in-process_order_status", $this->siteLangId) . "</small>";

                $fld = $frm->addSelectBox(
                    Labels::getLabel("LBL_Default_Shipping_Order_Status", $this->siteLangId),
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_default_child_order_status_when_an_order_is_marked_Shipped.", $this->siteLangId) . "</small>";

                $fld = $frm->addSelectBox(
                    Labels::getLabel("LBL_Default_Delivered_Order_Status", $this->siteLangId),
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_default_child_order_status_when_an_order_is_marked_delivered.", $this->siteLangId) . "</small>";

                $fld = $frm->addSelectBox(
                    Labels::getLabel("LBL_Default_Cancelled_Order_Status", $this->siteLangId),
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_default_child_order_status_when_an_order_is_marked_Cancelled.", $this->siteLangId) . "</small>";

                $fld = $frm->addSelectBox(
                    Labels::getLabel("LBL_Return_Requested_Order_Status", $this->siteLangId),
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_default_child_order_status_when_return_request_is_opened_on_any_order.", $this->siteLangId) . "</small>";

                $fld = $frm->addSelectBox(Labels::getLabel("LBL_Return_Request_Withdrawn_Order_Status", $this->siteLangId), 'CONF_RETURN_REQUEST_WITHDRAWN_ORDER_STATUS', $orderStatusArr, false, array(), '');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_default_child_order_status_when_return_request_is_withdrawn.", $this->siteLangId) . "</small>";

                $fld = $frm->addSelectBox(Labels::getLabel("LBL_Return_Request_Approved_Order_Status", $this->siteLangId), 'CONF_RETURN_REQUEST_APPROVED_ORDER_STATUS', $orderStatusArr, false, array(), '');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_default_child_order_status_when_return_request_is_accepted_by_the_Seller.", $this->siteLangId) . "</small>";

                $fld = $frm->addSelectBox(Labels::getLabel("LBL_Pay_At_Store_Order_Status", $this->siteLangId), 'CONF_PAY_AT_STORE_ORDER_STATUS', $orderStatusArr, false, array(), '');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_Pay_at_store_order_status.", $this->siteLangId) . "</small>";

                $fld = $frm->addSelectBox(Labels::getLabel("LBL_Cash_on_Delivery_Order_Status", $this->siteLangId), 'CONF_COD_ORDER_STATUS', $orderStatusArr, false, array(), '');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_Cash_on_delivery_order_status.", $this->siteLangId) . "</small>";

                $fld = $frm->addSelectBox(Labels::getLabel("LBL_Ready_For_Pickup_Order_Status", $this->siteLangId), 'CONF_PICKUP_READY_ORDER_STATUS', $orderStatusArr, false, array(), '');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_Ready_for_Pickup_order_status.", $this->siteLangId) . "</small>";

                $fld = $frm->addSelectBox(
                    Labels::getLabel("LBL_STATUS_USED_BY_SYSTEM_TO_MARK_ORDER_AS_COMPLETED", $this->siteLangId),
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_SET_THE_DEFAULT_CHILD_ORDER_STATUS_WHEN_AN_ORDER_IS_MARKED_COMPLETED.", $this->siteLangId) . "</small>";

                $returnAge = FatApp::getConfig("CONF_DEFAULT_RETURN_AGE", FatUtility::VAR_INT, 7);
                $fld = $frm->addIntegerField(Labels::getLabel("LBL_DEFAULT_RETURN_AGE_[Days]", $this->siteLangId), 'CONF_DEFAULT_RETURN_AGE', $returnAge);
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_IT_WILL_CONSIDERED_IF_NO_RETURN_AGE_IS_DEFINED_IN_SHOP_OR_SELLER_PRODUCT.", $this->siteLangId) . "</small>";

                $vendorOrderSelected = (!empty($arrValues['CONF_VENDOR_ORDER_STATUS'])) ? $arrValues['CONF_VENDOR_ORDER_STATUS'] : 0;

                $fld = $frm->addCheckBoxes(Labels::getLabel("LBL_Seller_Order_Statuses", $this->siteLangId), 'CONF_VENDOR_ORDER_STATUS', $orderStatusArr, $vendorOrderSelected, array('class' => 'list-inline'));
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_order_status_the_customer's_order_must_reach_before_the_order_starts_displaying_to_Sellers.", $this->siteLangId) . "</small>";

                $buyerOrderSelected = (!empty($arrValues['CONF_BUYER_ORDER_STATUS'])) ? $arrValues['CONF_BUYER_ORDER_STATUS'] : 0;
                $fld = $frm->addCheckBoxes(Labels::getLabel("LBL_Buyer_Order_Statuses", $this->siteLangId), 'CONF_BUYER_ORDER_STATUS', $orderStatusArr, $buyerOrderSelected, array('class' => 'list-inline'));
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_order_status_the_customer's_order_must_reach_before_the_order_starts_displaying_to_Buyers.", $this->siteLangId) . "</small>";

                $processingOrderSelected = (!empty($arrValues['CONF_PROCESSING_ORDER_STATUS'])) ? $arrValues['CONF_PROCESSING_ORDER_STATUS'] : 0;
                $fld = $frm->addCheckBoxes(Labels::getLabel("LBL_Processing_Order_Status", $this->siteLangId), 'CONF_PROCESSING_ORDER_STATUS', $orderStatusArr, $processingOrderSelected, array('class' => 'list-inline'));
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_order_status_the_customer's_order_must_reach_before_the_order_starts_stock_subtraction.", $this->siteLangId) . "</small>";

                $completeOrderSelected = (!empty($arrValues['CONF_COMPLETED_ORDER_STATUS'])) ? $arrValues['CONF_COMPLETED_ORDER_STATUS'] : 0;
                $fld = $frm->addCheckBoxes(Labels::getLabel("LBL_Completed_Order_Status", $this->siteLangId), 'CONF_COMPLETED_ORDER_STATUS', $orderStatusArr, $completeOrderSelected, array('class' => 'list-inline'));
                ...
                $fld = $frm->addCheckBoxes(Labels::getLabel("LBL_Feedback_ready_Order_Status", $this->siteLangId), 'CONF_REVIEW_READY_ORDER_STATUS', $orderStatusArr, $feedbackOrderSelected, array('class' => 'list-inline'));
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_order_status_the_customer's_order_must_reach_before_they_are_allowed_to_review_the_orders.", $this->siteLangId) . "</small>";
                ...
                $fld = $frm->addCheckBoxes(Labels::getLabel("LBL_Allow_Order_Cancellation_by_Buyers", $this->siteLangId), 'CONF_ALLOW_CANCELLATION_ORDER_STATUS', $orderStatusArr, $allowCancellationOrderSelected, array('class' => 'list-inline'));
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_order_status_the_customer's_order_must_reach_before_they_are_allowed_to_place_cancellation_request_on_orders.", $this->siteLangId) . "</small>";
                ...
                $fld = $frm->addCheckBoxes(Labels::getLabel("LBL_Allow_Order_Cancellation_by_Buyers_On_Digital", $this->siteLangId), 'CONF_DIGITAL_ALLOW_CANCELLATION_ORDER_STATUS', $orderStatusArr, $allowCancellationOrderSelected, array('class' => 'list-inline'));
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_order_status_the_customer's_order_must_reach_before_they_are_allowed_to_place_cancellation_request_on_orders.", $this->siteLangId) . "</small>";
                ...
                $fld = $frm->addCheckBoxes(Labels::getLabel("LBL_Allow_Return/Exchange", $this->siteLangId), 'CONF_RETURN_EXCHANGE_READY_ORDER_STATUS', $orderStatusArr, $returnExchageOrderSelected, array('class' => 'list-inline'));
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_order_status_the_customer's_order_must_reach_before_they_are_allowed_to_place_return/exchange_request_on_orders.", $this->siteLangId) . "</small>";

                $enableDigitalDownloads = (!empty($arrValues['CONF_ENABLE_DIGITAL_DOWNLOADS'])) ? $arrValues['CONF_ENABLE_DIGITAL_DOWNLOADS'] : 0;
                $fld = $frm->addCheckBoxes(Labels::getLabel("LBL_Enable_Digital_Download", $this->siteLangId), 'CONF_ENABLE_DIGITAL_DOWNLOADS', $orderStatusArr, $enableDigitalDownloads, array('class' => 'list-inline'));
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_order_status_the_customer's_order_must_reach_before_they_are_allowed_to_access_their_downloadable_Products.", $this->siteLangId) . "</small>";
                ...
                $fld = $frm->addCheckBoxes(Labels::getLabel("LBL_Order_statuses_to_allow_to_attach_more_files_with_order_product", $this->siteLangId), 'CONF_ALLOW_FILES_TO_ADD_WITH_ORDER_STATUSES', $orderStatusArr, $statusesToAttachMoreFiles, array('class' => 'list-inline'));
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_order_statuses_to_allow_seller_or_admin_to_attach_more_files_with_order_products", $this->siteLangId) . "</small>";

                $badgeCountOrderSelected = (!empty($arrValues['CONF_BADGE_COUNT_ORDER_STATUS'])) ? $arrValues['CONF_BADGE_COUNT_ORDER_STATUS'] : 0;
                $fld = $frm->addCheckBoxes(Labels::getLabel("LBL_Order_Statuses_to_calculate_badge_count_(For_Admin)", $this->siteLangId), 'CONF_BADGE_COUNT_ORDER_STATUS', $orderStatusArr, $badgeCountOrderSelected, array('class' => 'list-inline'));
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Order_Statuses_to_calculate_badge_count_for_seller_orders_in_admin_left_navigation_panel", $this->siteLangId) . "</small>";
                ...
                $fld = $frm->addCheckBoxes(Labels::getLabel("LBL_Products_On_Order_Stage(For_Seller_Inventory_Report)", $this->siteLangId), 'CONF_PRODUCT_IS_ON_ORDER_STATUSES', $orderStatusArr, $productOnOrderStatusesSelected, array('class' => 'list-inline'));
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Products_are_in_On_Order_Used_on_Seller_Dashboard_Products_Inventory_Stock_Status_Report", $this->siteLangId) . "</small>";
            ...
            case Configurations::FORM_CART_WISHLIST:
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_interval_in_days_to_send_auto_notification_alert_to_buyer_for_products_in_cart.", $this->siteLangId) . "</small>";

                $fld = $frm->addIntegerField(Labels::getLabel("LBL_Set_Notification_Count_to_be_Sent", $this->siteLangId), 'CONF_SENT_CART_REMINDER_COUNT', '');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_how_many_notifications_will_be_sent_to_buyer.", $this->siteLangId) . "</small>";

                $frm->addHtml('', 'Wishlist', '<div class="separator separator-dashed my-2"></div><h3 class="form-section-head">' . Labels::getLabel("LBL_Wishlist", $this->siteLangId) . '</h3>');

                $fld = $frm->addIntegerField(Labels::getLabel("LBL_Reminder_Interval_For_Products_In_Wishlist_[Days]", $this->siteLangId), 'CONF_REMINDER_INTERVAL_PRODUCTS_IN_WISHLIST', '');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_interval_in_days_to_send_auto_notification_alert_to_buyer_for_products_in_Wishlist.", $this->siteLangId) . "</small>";

                $fld = $frm->addIntegerField(Labels::getLabel("LBL_Set_Notification_Count_to_be_Sent", $this->siteLangId), 'CONF_SENT_WISHLIST_REMINDER_COUNT', '');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_how_many_notifications_will_be_sent_to_buyer.", $this->siteLangId) . "</small>";
            ...
            case Configurations::FORM_COMMISSION:
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_maximum_commission/Fees_that_will_be_charged_on_a_particular_product.", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("LBL_Commission_charged_including_shipping_charges", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("LBL_Commission_charged_including_tax_charges", $this->siteLangId) . '</small>';
                break;
            ...
            case Configurations::FORM_AFFILIATE:
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Automatically_approve_any_new_affiliates_who_sign_up.", $this->siteLangId) . "</small>";
                ...
                $fld->requirements()->setInt();
                $fld->htmlAfterField = '<small>' . Labels::getLabel('LBL_Affiliate_will_get_commission_when_new_registration_is_received_through_affiliate.', $this->siteLangId) . '</small>';

                $cpagesArr = ContentPage::getPagesForSelectBox($this->siteLangId);
                $fld = $frm->addSelectBox(Labels::getLabel('LBL_Affiliate_Terms', $this->siteLangId), 'CONF_AFFILIATE_TERMS_AND_CONDITIONS_PAGE', $cpagesArr, '', array(), '');
                $fld->htmlAfterField = '<small>' . Labels::getLabel('LBL_Forces_affiliate_to_agree_to_terms_before_an_affiliate_account_can_be_created.', $this->siteLangId) . '</small>';

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Referrer_Url/link_Validity_Period", $this->siteLangId), 'CONF_AFFILIATE_REFERRER_URL_VALIDITY');
                $fld->htmlAfterField = '<small>' . Labels::getLabel('LBL_Days,_After_Which_Referrer_Url_Is_Expired.(Cookie_Data_on_landed_user)', $this->siteLangId) . '</small>';

                $fld = $frm->addRadioButtons(
                    Labels::getLabel("LBL_New_Affiliate_Alert_Mail", $this->siteLangId),
                    'CONF_NOTIFY_ADMIN_AFFILIATE_REGISTRATION',
                    applicationConstants::getYesNoArr($this->siteLangId),
                    '',
                    array('class' => 'list-inline')
                );
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Send_an_email_to_the_store_owner_when_a_new_affiliate_is_registered.", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_affiliate_user_need_to_verify_their_email_address", $this->siteLangId) . " </small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_On_enabling_this_feature,_affiliate_will_receive_a_welcome_e-mail_after_registration.", $this->siteLangId) . "</small>";
            ...
            case Configurations::FORM_REWARD_POINTS:
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_how_many_rewards_points_equal_to", $this->siteLangId) . "[" . $this->siteDefaultCurrencyCode . "]</small>";
                $fld = $frm->addIntegerField(Labels::getLabel("LBL_Minimum_Reward_Point_Required_To_Use", $this->siteLangId), 'CONF_MIN_REWARD_POINT');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_minimun_reward_points_required_user_to_avail_discount_during_checkout", $this->siteLangId) . " .</small>";

                $fld = $frm->addIntegerField(Labels::getLabel("LBL_Maximum_Reward_Point", $this->siteLangId), 'CONF_MAX_REWARD_POINT');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_maximum_reward_points_limit_to_avail_discount_during_checkout", $this->siteLangId) . "</small>";
                ...
                $fld11->htmlAfterField = "<small>" . Labels::getLabel("MSG_Buyer_will_reward_point_on_every_purchase_as_defined_settings", $this->siteLangId) . "</small>";

                $fld = $frm->addIntegerField(Labels::getLabel("LBL_Reward_Point_Validity", $this->siteLangId), 'CONF_REWARDS_VALIDITY_ON_PURCHASE');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Reward_Point_Validity_in_days_from_date_of_credit", $this->siteLangId) . "</small>";
                ...
                $frm->addRadioButtons(
                    Labels::getLabel("LBL_Enable_birthday_discount", $this->siteLangId),
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_User_get_this_reward_points_on_his_birthday.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextBox(Labels::getLabel("LBL_reward_Points_Validity", $this->siteLangId), 'CONF_BIRTHDAY_REWARD_POINTS_VALIDITY');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Reward_Points_validity_in_days_from_the_date_of_credit._Please_leave_it_blank_if_you_don't_want_reward_points_to_expire.", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Enable_Buying_in_an_year_reward_points_module", $this->siteLangId) . "</small>";
                ...
                $fld = $frm->addTextBox(Labels::getLabel("LBL_Reward_Points", $this->siteLangId), 'CONF_BUYING_IN_AN_YEAR_REWARD_POINTS');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_User_get_this_reward_points_on_min_buying_value_in_an_year", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Reward_Points_validity_in_days_from_the_date_of_credit", $this->siteLangId) . "</small>";
            ...
            case Configurations::FORM_REVIEWS:
                ...
                $fld = $frm->addSelectBox(
                    Labels::getLabel("LBL_Default_Review_Status", $this->siteLangId),
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Set_the_default_review_order_status_when_a_new_review_is_placed", $this->siteLangId) . "</small>";
            ...
            case Configurations::FORM_EMAIL:
                $fld = $frm->addEmailField(Labels::getLabel("LBL_From_Email", $this->siteLangId), 'CONF_FROM_EMAIL');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Required_for_sending_emails", $this->siteLangId) . "</small>";
                $fld = $frm->addEmailField(Labels::getLabel("LBL_Reply_to_Email_Address", $this->siteLangId), 'CONF_REPLY_TO_EMAIL');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Required_for_email_headers_-_user_can_reply_to_this_email", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Email_id_to_contact_site_owner", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Any_additional_emails_you_want_to_receive_the_alert_email", $this->siteLangId) . "</small>";

                break;

            case Configurations::FORM_LIVE_CHAT:
                $fld = $frm->addRadioButtons(
                    Labels::getLabel("LBL_Activate_Live_Chat", $this->siteLangId),
                    'CONF_ENABLE_LIVECHAT',
                    applicationConstants::getYesNoArr($this->siteLangId),
                    '',
                    array('class' => 'list-inline')
                );
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Activate_3rd_Party_Live_Chat.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextarea(Labels::getLabel("LBL_Live_Chat_Code", $this->siteLangId), 'CONF_LIVE_CHAT_CODE');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_live_chat_script/code_provided_by_the_3rd_party_API_for_integration.", $this->siteLangId) . "</small>";
            ...
            case Configurations::FORM_THIRD_PARTY_API:
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_api_key_used_in_push_notifications.", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_THIS_IS_THE_FACEBOOK_PIXEL_ID_USED_IN_TRACK_EVENTS.", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_API_key_provided_by_Engagespot.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextarea(Labels::getLabel("LBL_Engagespot_Code", $this->siteLangId), 'CONF_ENGAGESPOT_PUSH_NOTIFICATION_CODE');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_code_provided_by_the_engagespot_for_integration.", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_Google_map_api_key_used_to_get_user_current_location.", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Please_select_the_system_you_wish_to_use_for_email_marketing.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Mailchimp_Key", $this->siteLangId), 'CONF_MAILCHIMP_KEY');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_Mailchimp's_application_key_used_in_subscribe_and_send_newsletters.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Mailchimp_List_ID", $this->siteLangId), 'CONF_MAILCHIMP_LIST_ID');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_Mailchimp's_subscribers_List_ID.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextarea(Labels::getLabel("LBL_Aweber_Signup_Form_Code", $this->siteLangId), 'CONF_AWEBER_SIGNUP_CODE');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Enter_the_newsletter_signup_code_received_from_Aweber", $this->siteLangId) . "</small>";

                $frm->addHtml('', 'Analytics', '<div class="separator separator-dashed my-2"></div><h3 class="form-section-head">' . Labels::getLabel("LBL_Google_Analytics", $this->siteLangId) . '</h3>');
                $fld = $frm->addTextBox(Labels::getLabel("LBL_Client_Id", $this->siteLangId), 'CONF_ANALYTICS_CLIENT_ID');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_application_Client_Id_used_in_Analytics_dashboard.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Secret_Key", $this->siteLangId), 'CONF_ANALYTICS_SECRET_KEY');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_application_secret_key_used_in_Analytics_dashboard.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Analytics_Id", $this->siteLangId), 'CONF_ANALYTICS_ID');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_Google_Analytics_ID._Ex._UA-xxxxxxx-xx.", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_application_Site_key_used_for_Google_Recaptcha.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Secret_Key", $this->siteLangId), 'CONF_RECAPTCHA_SECRETKEY');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_application_Secret_key_used_for_Google_Recaptcha.", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_MICROSOFT_TRANSLATOR_TEXT_API_3.0_SUBSCRIPTION_KEY.", $this->siteLangId) . "</small>";
                ...
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_key_provided_by_JW_PLAYER", $this->siteLangId) . "</small>";
                /* JW player Settings */
                break;
            case Configurations::FORM_REFERAL:
                $fld = $frm->addRadioButtons(
                    Labels::getLabel("LBL_Enable_Referral_Module", $this->siteLangId),
                ...
                $fld->requirements()->setIntPositive();
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Referrers_get_this_reward_points_when_their_referrals_(friends)_will_register.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Referrer_Reward_Points_Validity", $this->siteLangId), 'CONF_REGISTRATION_REFERRER_REWARD_POINTS_VALIDITY');
                $fld->requirements()->setIntPositive();
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Rewards_points_validity_in_days_from_the_date_of_credit", $this->siteLangId) . "</small>";

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Referral_Reward_Points", $this->siteLangId), 'CONF_REGISTRATION_REFERRAL_REWARD_POINTS');
                $fld->requirements()->setIntPositive();
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Referrals_get_this_reward_points_when_they_register_through_referrer.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Referral_Reward_Points_Validity", $this->siteLangId), 'CONF_REGISTRATION_REFERRAL_REWARD_POINTS_VALIDITY');
                $fld->requirements()->setIntPositive();
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Rewards_points_validity_in_days_from_the_date_of_credit", $this->siteLangId) . "</small>";
                ...
                $fld->requirements()->setIntPositive();
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Referrers_get_this_reward_points_when_their_referrals_(friends)_will_make_first_purchase.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Referrer_Reward_Points_Validity", $this->siteLangId), 'CONF_SALE_REFERRER_REWARD_POINTS_VALIDITY');
                $fld->requirements()->setIntPositive();
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Rewards_points_validity_in_days_from_the_date_of_credit", $this->siteLangId) . "</small>";

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Referral_Reward_Points", $this->siteLangId), 'CONF_SALE_REFERRAL_REWARD_POINTS');
                $fld->requirements()->setIntPositive();
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Referrals_get_this_reward_points_when_they_will_make_first_purchase_through_their_referrers.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Rewards_points_validity_in_days", $this->siteLangId), 'CONF_SALE_REFERRAL_REWARD_POINTS_VALIDITY');
                $fld->requirements()->setIntPositive();
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_NOTE:Rewards_points_validity_in_days_from_the_date_of_credit", $this->siteLangId) . "</small>";
            ...
            case Configurations::FORM_DISCOUNT:
                ...
                $fld =  $frm->addTextBox(Labels::getLabel("LBL_Discount_value", $this->siteLangId), 'CONF_FIRST_TIME_BUYER_COUPON_DISCOUNT_VALUE');
                $fld->requirements()->setPositive();

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Minimum_order_value", $this->siteLangId), 'CONF_FIRST_TIME_BUYER_COUPON_MIN_ORDER_VALUE');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Minimum_order_value_on_which_the_coupon_can_be_applied.", $this->siteLangId) . "</small>";
                $fld->requirements()->setPositive();


                $fld = $frm->addTextBox(Labels::getLabel("LBL_Max_Discount_Value", $this->siteLangId), 'CONF_FIRST_TIME_BUYER_COUPON_MAX_DISCOUNT_VALUE');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Max_discount_value_user_can_get_by_using_this_coupon.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Discount_Coupon_Validity", $this->siteLangId), 'CONF_FIRST_TIME_BUYER_COUPON_VALIDITY');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_Coupon_validity_in_days_from_the_date_of_credit", $this->siteLangId) . "</small>";
            ...
            case Configurations::FORM_SUBSCRIPTION:
                ...
                $enable_subscption_module_fld->htmlAfterField = '<small>' . Labels::getLabel('LBL_Seller_Needs_to_Purchase_the_subscrption_before_listing_products', $this->siteLangId) . '</small>';
                $enable_subscption_module_fld = $frm->addRadioButtons(
                ...
                $enable_subscption_module_fld->htmlAfterField = '<small>' . Labels::getLabel('LBL_Subscription_Payment_will_be_adjusted_While_Upgrading/downgrading_plan', $this->siteLangId) . '</small>';
            ...
            case Configurations::FORM_SYSTEM:
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("LBL_NOTE:_After_how_much_seconds_system_message_should_be_close", $this->siteLangId) . '.</small>';
                $fld->requirements()->setInt();
                break;
            case Configurations::FORM_PPC:
                $fld = $frm->addFloatField(Labels::getLabel('LBL_Minimum_Wallet_Balance', $this->siteLangId), 'CONF_PPC_MIN_WALLET_BALANCE');
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Minimum_wallet_balance_to_start_promotion", $this->siteLangId) . '</small>';

                $fld = $frm->addTextBox(Labels::getLabel('LBL_Days_Interval_to_Charge_Wallet', $this->siteLangId), 'CONF_PPC_WALLET_CHARGE_DAYS_INTERVAL');
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Days_Interval_to_Charge_Wallet", $this->siteLangId) . '</small>';

                $fld = $frm->addFloatField(Labels::getLabel('LBL_Cost_Per_Click_(product)', $this->siteLangId), 'CONF_CPC_PRODUCT');
                $fld->requirements()->setCompareWith('CONF_PPC_MIN_WALLET_BALANCE', 'lt');
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_PPC_cost_per_click_for_Product", $this->siteLangId) . '</small>';

                $fld = $frm->addFloatField(Labels::getLabel('LBL_Cost_Per_Click_(shop)', $this->siteLangId), 'CONF_CPC_SHOP');
                $fld->requirements()->setCompareWith('CONF_PPC_MIN_WALLET_BALANCE', 'lt');
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_PPC_cost_per_click_for_shop", $this->siteLangId) . '</small>';

                $fld = $frm->addFloatField(Labels::getLabel('LBL_Cost_Per_Click_(slide)', $this->siteLangId), 'CONF_CPC_SLIDES');
                $fld->requirements()->setCompareWith('CONF_PPC_MIN_WALLET_BALANCE', 'lt');
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_PPC_cost_per_click_for_slide", $this->siteLangId) . '</small>';

                $fld = $frm->addTextBox(Labels::getLabel('LBL_PPC_products_count_home_page', $this->siteLangId), 'CONF_PPC_PRODUCTS_HOME_PAGE');
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_how_many_PPC_products_shown_on_home_page", $this->siteLangId) . '</small>';

                $fld = $frm->addTextBox(Labels::getLabel('LBL_PPC_shops_count_home_page', $this->siteLangId), 'CONF_PPC_SHOPS_HOME_PAGE');
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_how_many_PPC_shops_shown_on_home_page", $this->siteLangId) . '</small>';
                $fld = $frm->addTextBox(Labels::getLabel('LBL_PPC_slides_count_home_page', $this->siteLangId), 'CONF_PPC_SLIDES_HOME_PAGE');
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_how_many_PPC_slides_shown_on_home_page", $this->siteLangId) . '</small>';
                $fld = $frm->addTextBox(Labels::getLabel('LBL_PPC_Clicks_Count_Time_Interval(Minutes)', $this->siteLangId), 'CONF_PPC_CLICK_COUNT_TIME_INTERVAL');
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Set_time_interval_to_calculate_no._of_click_from_one_user_for_each_promotion", $this->siteLangId) . '</small>';
            ...
            case Configurations::FORM_SERVER:
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("LBL_NOTE:_To_use_SSL,_check_with_your_host_if_a_SSL_certificate_is_installed_and_enable_it_from_here.", $this->siteLangId) . '.</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("LBL_NOTE:_Enable_Maintenance_Mode_Text", $this->siteLangId) . '.</small>';
            ...
            case Configurations::FORM_IMPORT_EXPORT:
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_brand_id_instead_of_brand_identifier_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_category_id_instead_of_category_identifier_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_catalog_product_id_instead_of_catalog_product_identifier_in_worksheets", $this->siteLangId) . '</small>';

                $fld = $frm->addCheckBox(Labels::getLabel("LBL_Use_user_id_instead_of_username", $this->siteLangId), 'CONF_USE_USER_ID', 1, array(), false, 0);
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_user_id_instead_of_username_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_option_id_instead_of_option_identifier_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_option_value_id_instead_of_option_value_identifier_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_tag_id_instead_of_tag_identifier_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_tax_category_id_instead_of_tax_identifier_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_product_type_id_instead_of_product_type_identifier_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_dimension_unit_id_instead_of_dimension_unit_identifier_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_weight_unit_id_instead_of_weight_unit_identifier_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_language_id_instead_of_language_code_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_currency_id_instead_of_currency_code_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_Product_condition_id_instead_of_condition_identifier_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_persent_or_flat_condition_id_instead_of_identifier_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_country_id_instead_of_country_code_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_state_id_instead_of_state_identifier_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_policy_point_id_instead_of_policy_point_identifier_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_shipping_company_id_instead_of_shipping_company_identifier_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_policy_point_type_id_instead_of_policy_point_type_identifier_in_worksheets", $this->siteLangId) . '</small>';
                ...
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_shipping_duration_id_instead_of_shipping_duration_identifier_in_worksheets", $this->siteLangId) . '</small>';

                $fld = $frm->addCheckBox(Labels::getLabel("LBL_Use_1_for_yes_0_for_no", $this->siteLangId), 'CONF_USE_O_OR_1', 1, array(), false, 0);
                $fld->htmlAfterField = '<small>' . Labels::getLabel("MSG_Use_1_for_yes_0_for_no_for_status_type_data", $this->siteLangId) . '</small>';
                break;
        }
    ...
    private function getLangForm($type, $langId)
    {
        ...
        switch ($type) {
            case Configurations::FORM_GENERAL:
                $frm->addTextBox(Labels::getLabel("LBL_Site_Name", $this->siteLangId), 'CONF_WEBSITE_NAME_' . $langId);
                $frm->addTextBox(Labels::getLabel("LBL_Site_Owner", $this->siteLangId), 'CONF_SITE_OWNER_' . $langId);
            ...
            case Configurations::FORM_SHARING:
                $frm->addHtml('', 'ShareAndEarn', '<h3 class="form-section-head">' . Labels::getLabel('LBL_Share_and_Earn_Settings', $this->siteLangId) . '</h3>');

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Facebook_APP_ID", $this->siteLangId), 'CONF_FACEBOOK_APP_ID');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_application_ID_used_in_post.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Facebook_App_Secret", $this->siteLangId), 'CONF_FACEBOOK_APP_SECRET');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_Facebook_secret_key_used_for_authentication_and_other_Facebook_related_plugins_support.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextbox(Labels::getLabel("LBL_Facebook_Post_Title", $this->siteLangId), 'CONF_SOCIAL_FEED_FACEBOOK_POST_TITLE_' . $langId);
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_title_shared_on_facebook", $this->siteLangId) . "</small>";
                $fld = $frm->addTextbox(Labels::getLabel("LBL_Facebook_Post_Caption", $this->siteLangId), 'CONF_SOCIAL_FEED_FACEBOOK_POST_CAPTION_' . $langId);
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_caption_shared_on_facebook", $this->siteLangId) . "</small>";
                $fld = $frm->addTextarea(Labels::getLabel("LBL_Facebook_Post_Description", $this->siteLangId), 'CONF_SOCIAL_FEED_FACEBOOK_POST_DESCRIPTION_' . $langId);
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_description_shared_on_facebook", $this->siteLangId) . "</small>";

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Twitter_APP_KEY", $this->siteLangId), 'CONF_TWITTER_API_KEY');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_application_ID_used_in_post.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextBox(Labels::getLabel("LBL_Twitter_App_Secret", $this->siteLangId), 'CONF_TWITTER_API_SECRET');
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_is_the_Twitter_secret_key_used_for_authentication_and_other_Twitter_related_plugins_support.", $this->siteLangId) . "</small>";

                $fld = $frm->addTextarea(Labels::getLabel("LBL_Twitter_Post_Description", $this->siteLangId), 'CONF_SOCIAL_FEED_TWITTER_POST_TITLE' . $langId);
                $fld->htmlAfterField = "<small>" . Labels::getLabel("LBL_This_description_shared_on_twitter", $this->siteLangId) . "</small>";
                break;
            ...
            case Configurations::FORM_MEDIA:
                ...
                $ul->htmlAfterField .= ' </div></div><input type="file" onChange="popupImage(this)" name="social_feed_image" id="social_feed_image" data-min_width = "160" data-min_height = "240" data-file_type=' . AttachedFile::FILETYPE_SOCIAL_FEED_IMAGE . ' value="Upload file"><small>' . Labels::getLabel('LBL_Dimensions', $this->siteLangId) . ' 160*240</small></div>';
                ...
                $ul->htmlAfterField .= '<input type="file" onChange="popupImage(this)" name="payment_page_logo" id="payment_page_logo" data-min_width = "150" data-min_height = "150" data-file_type=' . AttachedFile::FILETYPE_PAYMENT_PAGE_LOGO . ' value="Upload file"><small>' . Labels::getLabel('MSG_PLEASE_UPLOAD_WHITE_PNG_IMAGE', $this->siteLangId) . '</small></div>';
                ...
                $ul->htmlAfterField .= ' </div></div><input type="file" onChange="popupImage(this)" name="watermark_image" id="watermark_image" data-min_width = "168" data-min_height = "37" data-file_type=' . AttachedFile::FILETYPE_WATERMARK_IMAGE . ' value="Upload file"><small>' . Labels::getLabel('LBL_Dimensions', $this->siteLangId) . ' 168*37</small></div>';
