...
class ConfigurationsController extends AdminBaseController
{
    ...
    public function setup()
    {
        ...
        if (isset($post["CONF_SEND_SMTP_EMAIL"]) && $post["CONF_SEND_EMAIL"] && $post["CONF_SEND_SMTP_EMAIL"] && (($post["CONF_SEND_SMTP_EMAIL"] != FatApp::getConfig("CONF_SEND_SMTP_EMAIL")) || ($post["CONF_SMTP_HOST"] != FatApp::getConfig("CONF_SMTP_HOST")) || ($post["CONF_SMTP_PORT"] != FatApp::getConfig("CONF_SMTP_PORT")) || ($post["CONF_SMTP_USERNAME"] != FatApp::getConfig("CONF_SMTP_USERNAME")) || ($post["CONF_SMTP_SECURE"] != FatApp::getConfig("CONF_SMTP_SECURE")) || ($post["CONF_SMTP_PASSWORD"] != FatApp::getConfig("CONF_SMTP_PASSWORD")))) {
            ...
            if (EmailHandler::sendSmtpTestEmail($this->adminLangId, $smtp_arr)) {
                ...
                LibHelper::exitWithError(Labels::getLabel("LBL_SMTP_settings_provided_is_invalid_or_unable_to_send_email_so_we_have_not_saved_SMTP_settings", $this->adminLangId), true);
            }
        }
        ...
        if (isset($post['CONF_USE_SSL']) && $post['CONF_USE_SSL'] == 1) {
            if (!$this->isSslEnabled()) {
                if ($post['CONF_USE_SSL'] != FatApp::getConfig('CONF_USE_SSL')) {
                    LibHelper::exitWithError(Labels::getLabel('MSG_SSL_NOT_INSTALLED_FOR_WEBSITE_Try_to_Save_data_without_Enabling_ssl', $this->adminLangId), true);
                }
        ...
        if (array_key_exists('CONF_CURRENCY', $post)) {
            ...
            if (empty($data) || ($data['currency_value'] * 1) != 1) {
                LibHelper::exitWithError(Labels::getLabel('MSG_Please_set_default_currency_value_to_1', $this->adminLangId), true);
            }
        }
    ...
    private function getForm($type, $arrValues = array())
    {
        ...
        switch ($type) {
            ...
            case Configurations::FORM_LOCAL:
                ...
                $frm->addRequiredField(Labels::getLabel("LBL_Postal_Code", $this->adminLangId), 'CONF_ZIP_CODE');
            ...
            case Configurations::FORM_PRODUCT:
                $frm->addHtml('', 'Product', '<h3 class="form-section-head">' . Labels::getLabel('LBL_Product', $this->adminLangId) . '</h3>');
            ...
            case Configurations::FORM_AFFILIATE:
                /* Affiliate Accounts[ */
                $frm->addHtml('', Labels::getLabel('LBL_Affiliate_Accounts', $this->adminLangId), '<h3 class="form-section-head">' . Labels::getLabel("LBL_Affiliate_Accounts", $this->adminLangId) . '</h3>');
            ...
            case Configurations::FORM_REWARD_POINTS:
                $frm->addHtml('', 'Reward', '<h3 class="form-section-head">' . Labels::getLabel("LBL_Reward_Points", $this->adminLangId) . '</h3>');
            ...
            case Configurations::FORM_REVIEWS:
                $frm->addHtml('', 'Reviews', '<h3 class="form-section-head">' . Labels::getLabel("LBL_Reviews", $this->adminLangId) . '</h3>');
            ...
            case Configurations::FORM_THIRD_PARTY_API:
                $frm->addHtml('', 'GooglePushNotification', '<h3 class="form-section-head">' . Labels::getLabel("LBL_GOOGLE_PUSH_NOTIFICATION", $this->adminLangId) . '</h3>');
            ...
            case Configurations::FORM_DISCOUNT:
                $frm->addHtml('', 'Birthday Discount', '<h3 class="form-section-head">' . Labels::getLabel("LBL_First_time_buyers_discount_coupon", $this->adminLangId) . '</h3>');
    ...
    private function getLangForm($type, $langId)
    {
        ...
        switch ($type) {
            ...
            case Configurations::FORM_SHARING:
                $frm->addHtml('', 'ShareAndEarn', '<h3 class="form-section-head">' . Labels::getLabel('LBL_Share_and_Earn_Settings', $this->adminLangId) . '</h3>');
            ...
            case Configurations::FORM_MEDIA:
                ...
                $ul->htmlAfterField .= '<div class="col-md-4 mb-5"><h3 class="form-section-head">' . Labels::getLabel('LBL_Select_Admin_Logo', $this->adminLangId) . ' </h3> <div class="logoWrap"><div class="uploaded--image">';
    ...
    {
        if ($fileType == '') {
            LibHelper::exitWithError(Labels::getLabel('LBL_Invalid_Request', $this->adminLangId), true);
        }
        $target_dir = CONF_UPLOADS_PATH;
        if ($fileType == 'bing') {
            $path_filename = $target_dir . 'BingSiteAuth.xml';
