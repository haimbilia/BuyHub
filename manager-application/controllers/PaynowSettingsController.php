...
class PaynowSettingsController extends PaymentMethodSettingsController
{
    public static function form(int $langId)
    {
        ...
        $frm->addTextBox(Labels::getLabel('FRM_API_ACCESS_KEY', $langId), 'api_access_key');
        $fld1 = new FormFieldRequirement('api_access_key', Labels::getLabel('FRM_API_ACCESS_KEY', $langId));
        $fld1->setRequired(false);
        $reqFld1 = new FormFieldRequirement('api_access_key', Labels::getLabel('FRM_API_ACCESS_KEY', $langId));
        $reqFld1->setRequired(true);

        $frm->addTextBox(Labels::getLabel('FRM_SIGNATURE_CALCULATION_KEY', $langId), 'signature_calculation_key');
        $fld2 = new FormFieldRequirement('signature_calculation_key', Labels::getLabel('FRM_SIGNATURE_CALCULATION_KEY', $langId));
        $fld2->setRequired(false);
        $reqFld2 = new FormFieldRequirement('signature_calculation_key', Labels::getLabel('FRM_SIGNATURE_CALCULATION_KEY', $langId));
        $reqFld2->setRequired(true);


        $frm->addTextBox(Labels::getLabel('FRM_API_ACCESS_KEY', $langId), 'live_api_access_key');
        $liveFld1 = new FormFieldRequirement('live_api_access_key', Labels::getLabel('FRM_API_ACCESS_KEY', $langId));
        $liveFld1->setRequired(false);
        $reqLiveFld1 = new FormFieldRequirement('live_api_access_key', Labels::getLabel('FRM_API_ACCESS_KEY', $langId));
        $reqLiveFld1->setRequired(true);

        $frm->addTextBox(Labels::getLabel('FRM_SIGNATURE_CALCULATION_KEY', $langId), 'live_signature_calculation_key');
        $liveFld2 = new FormFieldRequirement('live_signature_calculation_key', Labels::getLabel('FRM_SIGNATURE_CALCULATION_KEY', $langId));
        $liveFld2->setRequired(false);
        $reqLiveFld2 = new FormFieldRequirement('live_signature_calculation_key', Labels::getLabel('FRM_SIGNATURE_CALCULATION_KEY', $langId));
