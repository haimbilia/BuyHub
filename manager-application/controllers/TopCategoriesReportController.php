...
class TopCategoriesReportController extends ListingBaseController
{
    ...
    public function search($export = false)
    {
        ...
        $srch = new ProductCategorySearch($this->siteLangId, false, false, false, false);
        ...
        if ($export == 'export') {
            ...
            $catTreeAssocArr = $catObj->getProdCatTreeStructure(0, $this->siteLangId, '', 0, '', false, false, true);
            /* ] */

            $rs = $srch->getResultSet();
            $sheetData = array();
            $arr = array(Labels::getLabel('LBL_Category', $this->siteLangId), Labels::getLabel('LBL_Sold_Quantity', $this->siteLangId), Labels::getLabel('LBL_Favorites', $this->siteLangId));
            ...
            if ($orderBy == "DESC") {
                CommonHelper::convertToCsv($sheetData, Labels::getLabel('LBL_Top_Categories_Report', $this->siteLangId) . ' ' . date("d-M-Y") . '.csv', ',');
                exit;
            } else {
                CommonHelper::convertToCsv($sheetData, Labels::getLabel('LBL_Bad_Categories_Report', $this->siteLangId) . ' ' . date("d-M-Y") . '.csv', ',');
                exit;
            }
        } else {
            /* Cat Tree Structure Assoc Arr[ */
            $catObj = new ProductCategory();
            $catTreeAssocArr = $catObj->getProdCatTreeStructure(0, $this->siteLangId, '', 0, '', false, false);
            /* ] */
