...
<?php if(count($couponPlans)>0){
	...
	foreach($couponPlans as $plan){
		...
		$lis .= '<span >' . DiscountCoupons::getPlanTitle($plan,$siteLangId).' ('.DiscountCoupons::getPlanTitle($plan,$siteLangId).')'.'<input type="hidden" value="'.$plan['spplan_id'].'"  name="plans[]"></span></li>';
	}
	echo $lis;
} ?>
</ul>