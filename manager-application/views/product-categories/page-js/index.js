...
(function() {
    ...
    categoryForm = function(prodCatId) {
        fcom.ajax(fcom.makeUrl('productCategories', 'form', [prodCatId]), '', function(res) {
            ...
            if (prodCatId > 0) {
                ...
                categoryImages(prodCatId, 'thumb', 1);
            }
        });
    }
    ...
    categoryImages = function(prodCatId, imageType, slide_screen, lang_id = 0) {
        fcom.ajax(fcom.makeUrl('ProductCategories', 'images', [prodCatId, imageType, lang_id, slide_screen]), '', function(t) {
            ...
            if (imageType == 'thumb') {
                $('#thumb-imageListingJs').html(t);
                var prodCatId = $("[name='prodcat_id']").val();
                if (prodCatId == 0) {
                    var thumbImageId = $("#thumb-imageListingJs li").attr('id');
                    var selectedLangId = $(".thumb-language-js").val();
                    $("[name='cat_thumb_image_id[" + selectedLangId + "]']").val(thumbImageId);
                }
    ...
    uploadCatImages = function(formData) {
        ...
        if (formData.get("file_type") == 'icon') {
            ...
            var imageType = 'icon';
        } else if (formData.get("file_type") == 'banner') {
    ...
    mediaForm = function(record_id) {
        ...
        fcom.ajax(fcom.makeUrl('ProductCategories', 'imagesForm', [record_id]), '', function(t) {
            ...
            if (record_id > 0) {
                ...
                categoryImages(record_id, 'thumb', 1);
            }
            fcom.removeLoader();
        });
    };
...
$(document).on('change', '.thumb-language-js', function() {
    var lang_id = $(this).val();
    var prodcat_id = $("input[name='prodcat_id']").val();
    var imageId = $("[name='cat_thumb_image_id[" + lang_id + "]']").val();
    if (prodcat_id == 0) {
        if (imageId > 0) {
            categoryImages(prodcat_id, 'thumb', 0, lang_id);
        } else {
            $("#thumb-imageListingJs").html('');
        }
    } else {
        categoryImages(prodcat_id, 'thumb', 0, lang_id);
    }
