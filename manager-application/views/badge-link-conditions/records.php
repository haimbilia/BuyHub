...
if ($records) {
    ...
    foreach ($records as $record) {
        ...
        if (BadgeLinkCondition::RECORD_TYPE_SHOP != $recordType) {
            $tr->appendElement('td')
                ->appendElement('a', array('class' => 'text-dark', 'href' => 'javascript:void(0)', 'title' => Labels::getLabel('LBL_REMOVE', $siteLangId), "onclick" => "removeBadgeLinkRecord(event, " . $badgeLinkCondId . "," . $record['badgelink_record_id'] . ")"), "<i class='icon ion-close'></i>", true);
        }
    ...
    if (BadgeLinkCondition::RECORD_TYPE_SHOP != $recordType) {
        $pagingArr = array('pageCount' => $pageCount, 'page' => $page, 'recordCount' => $recordCount, 'adminLangId' => $siteLangId, 'callBackJsFunc' => 'reloadRecordsList', 'arguments' => $badgeLinkCondId);
