...
if (!empty($images)) {
    foreach ($images as $afile_id => $row) {
        ...
        $imgUrl =  UrlHelper::getCachedUrl(UrlHelper::generateFileUrl('Image', 'collectionReal', array($row['afile_record_id'], $row['afile_lang_id'], "THUMB", 0, $row['afile_id']), CONF_WEBROOT_FRONT_URL) . $uploadedTime, CONF_IMG_CACHE_TIME, '.jpg');
?>
