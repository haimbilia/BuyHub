...
class AdminAuthentication extends FatModel
{
    ...
    public function login($username, $password, $ip)
    {
        ...
        if ($objUserAuthentication->isBruteForceAttempt($ip, $username)) {
            $this->error = Labels::getLabel('ERR_Login_attempt_limit_exceeded._Please_try_after_some_time.', $this->adminLangId);
            return false;
        }
        $db = FatApp::getDb();
        ...
        if (!$row = $db->fetch($rs)) {
            ...
            $this->error = Labels::getLabel('ERR_Invalid_Username', $this->adminLangId);
            return false;
        }
        ...
        if (false == password_verify($password, $row['admin_password'])) {
            $objUserAuthentication->logFailedAttempt($ip, $username);
            $this->error = Labels::getLabel('ERR_Invalid_Password', $this->adminLangId);
            return false;
        }
        // if (!empty($row['admin_password'])) {            
        //     if (false == password_verify($password, $row['admin_password'])) {
        ...
        /*if (strtolower($row['admin_username']) != strtolower($username) || $row['admin_password'] != $password) {
            $objUserAuthentication->logFailedAttempt($ip, $username);
            $this->error = Labels::getLabel('ERR_Invalid_Username_or_Password', $this->adminLangId);
            return false;
        }*/
        if ($row['admin_active'] !== applicationConstants::ACTIVE) {
            $objUserAuthentication->logFailedAttempt($ip, $username);
            $this->error = Labels::getLabel('ERR_Your_account_is_inactive.', $this->adminLangId);
            return false;
        }
        $row['admin_ip'] = $ip;
    ...
    {      
        ...
        $rs = $srch->getResultSet();
        if (!$row = $db->fetch($rs)) {
            $this->error = Labels::getLabel('ERR_Invalid_email_address_or_username!', $this->adminLangId);
            return false;
        }      
        return $row;
    }
    public function checkAdminEmail($email)
    {
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $this->error = Labels::getLabel('ERR_Invalid_email_address!', $this->adminLangId);
            return false;
        }
        $db = FatApp::getDb();
        $srch = new SearchBase('tbl_admin');
    ...
    public function checkAdminPwdResetRequest($admin_id)
    {
        ...
        $this->error = Labels::getLabel('ERR_Your_request_to_reset_password_has_already_been_placed_within_last_24_hours._Please_check_your_emails_or_retry_after_24_hours_of_your_previous_request', $this->adminLangId);
        return true;
    }
    ...
    public function checkResetLink($aId, $token)
    {
        ...
        if (intval($aId) < 1 || strlen($token) < 20) {
            $this->error = Labels::getLabel('ERR_Link_is_invalid_or_expired!', $this->adminLangId);
            return false;
        }
        $db = FatApp::getDb();
        $srch = new SearchBase('tbl_admin_password_reset_requests');
