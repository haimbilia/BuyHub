...
class Questions extends MyAppModel
{
    ...
    public static function getSearchObject($langId = 0, $active = true, $deleted = true)
    {
        ...
        if ($active == true) {
            $srch->addCondition('q.' . static::DB_TBL_PREFIX . 'active', '=', 'mysql_func_' . applicationConstants::ACTIVE, 'AND', true);
        }

        if ($deleted == true) {
            $srch->addCondition('q.' . static::DB_TBL_PREFIX . 'deleted', '=',  'mysql_func_' . applicationConstants::NO, 'AND', true);
        }
        return $srch;
    }
