    ...
    <li id="<?php echo $row['afile_id']; ?>">
        <div class="logoWrap">
            <div class="logothumb"> <img
                    src="<?php echo UrlHelper::generateUrl('image','customProduct', array($row['afile_record_id'], "THUMB",$row['afile_id']),CONF_WEBROOT_FRONT_URL); ?>"
                    title="<?php echo $row['afile_name'];?>" alt="<?php echo $row['afile_name'];?>">
                <?php echo ( $count == 1 ) ? '<small><strong>'.Labels::getLabel('LBL_Default_Image',$adminLangId).'</strong></small>' : '&nbsp;'; if($canEdit){ ?>
            ...
            <?php if(isset($imgTypesArr) && !empty($imgTypesArr[$row['afile_record_subid']])){
					...
					?>
            <?php } ?>
            <small class=""><strong> <?php echo Labels::getLabel('LBL_Language',$adminLangId); ?>:</strong>
