...
foreach ($arr_listing as $sn => $row) {
    ...
    foreach ($arr_flds as $key => $val) {
        ...
        switch ($key) {
            case 'select_all':
                $td->appendElement('plaintext', array(), '<label class="checkbox"><input class="selectItem--js" type="checkbox" name="rt_ids[]" value=' . $row['rt_id'] . '><i class="input-helper"></i></label>', true);
                break;
            case 'listserial':
            ...
            case 'action':
                if ($canEdit) {
                    ...
                    $td->appendElement('a', array('href' => 'javascript:void(0)', 'class' => 'btn btn-clean btn-sm btn-icon', 'title' => Labels::getLabel('LBL_Delete', $adminLangId), "onclick" => "removeRatingTypes(" . $row['rt_id'] . ")"), "<i class='fa fa-trash  icon'></i>", true);
                }
                break;
            default:
...
$frm = new Form('frmRatingTypesListing', array('id' => 'frmRatingTypesListing'));
