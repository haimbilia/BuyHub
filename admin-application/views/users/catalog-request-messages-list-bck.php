...
foreach ($messagesList as $sn=>$row){
	...
	foreach ($arr_flds as $key=>$val){
		...
		switch ($key){
			case 'image':
				$img = '<img title="'.$row['msg_user_name'].'" src = "'.UrlHelper::generateFileUrl('Image','User', array($row['scatrequestmsg_from_user_id'], 'THUMB', 1), CONF_WEBROOT_FRONT_URL).'" alt = "'.$row['msg_user_name'].'" >';
				$username = $row['msg_user_name'];
				
				if( $row['scatrequestmsg_from_admin_id'] ){
					$username = $row['admin_name'];
					$img = '<img title="'.$row['admin_name'].'" src = "'.UrlHelper::generateFileUrl('Image','siteAdminLogo', array( $adminLangId )).'" alt = "'.$row['admin_name'].'" >';
				}
