...
foreach ($arrListing as $sn => $row) {
    ...
    foreach ($arr_flds as $key => $val) {
        ...
        switch ($key) {
            case 'select_all':
                $td->appendElement('plaintext', array(), '<label class="checkbox"><input class="selectItemJs" type="checkbox" name="rop_ids[]" value=' . $row['rop_id'] . '><i class="input-helper"></i></label>', true);
                break;
            case 'listSerial':
