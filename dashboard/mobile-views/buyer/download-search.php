...
foreach ($digitalDownloads as $index => $row) {
    $digitalDownloads[$index]['product_image_url'] = UrlHelper::generateFullUrl('image', 'product', array($row['selprod_product_id'], "THUMB", $row['op_selprod_id'], 0, $siteLangId), CONF_WEBROOT_FRONTEND);
