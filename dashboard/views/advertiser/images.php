...
if (!empty($image) && isset($image['afile_id']) && $image['afile_id'] != -1) {   
	...
	switch ($promotionType) {
		case Promotion::TYPE_BANNER:
			$imgUrl = UrlHelper::generateFullUrl('Banner', 'BannerImage', array($image['afile_record_id'], $image['afile_lang_id'], $image['afile_screen'], ImageDimension::VIEW_THUMB), CONF_WEBROOT_FRONTEND);
			break;
		case Promotion::TYPE_SLIDES:
			$imgUrl = UrlHelper::generateFullUrl('Image', 'Slide', array($image['afile_record_id'], $image['afile_screen'], $image['afile_lang_id'], ImageDimension::VIEW_THUMB), CONF_WEBROOT_FRONTEND);
			break;
	}
