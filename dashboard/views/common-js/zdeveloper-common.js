...
$(window).scroll(function () {
	...
	if (body_height.top < scroll_position) $("body").addClass("scrolled");
...
$(document).ready(function () {
	...
	signInWithPhone = function (obj, flag) {
		...
		fcom.updateWithAjax(
			fcom.makeUrl("GuestUser", "loginForm", [], siteConstants.webrootfront),
			data,
			function (t) {
				if (0 < popup.length) {
					$.ykmodal(t.html, true);
	...
	getLoginOtp = function (obj) {
		...
		fcom.ajax(
			fcom.makeUrl("GuestUser", "getLoginOtp", [], siteConstants.webrootfront),
			data,
			function (t) {
				fcom.removeLoader();
				t = $.parseJSON(t);
...
function setSiteDefaultLang(langId) {
	$.cookie("defaultSiteLang", langId, {
		expires: 10,
		path: siteConstants.rooturl,
	});
	...
	if (input.files) {
		if (1 > $(placeToInsertImagePreview + " ul").length) {
			$(placeToInsertImagePreview).html(
				'<ul class="uploaded-media-list"></ul>'
			);
		}
		...
		for (i = 0; i < filesAmount; i++) {
			...
			reader.onload = function (event) {
				var htm =
					'<li class="uploaded-media-item"><div class="uploaded-file"><span class="uploaded-file__thumb"></span><button class="file-remove fileRemove--js" data-filefld="' +
					fileFldName +
		...
		$(".navLinkJs", liObj).each(function () {
			...
			if (-1 < search || -1 < parentSearch) {
				...
				if (parentTextPos >= 0) {
					...
					var replaceWith = "<mark>" + filter_text + "</mark>";
					parentTextEle.html(
						orignalParentText.replace(filter_text, replaceWith)
					);
				}
			}
		});
...
$(document).on("change", ".multipleImgs--js", function () {
	...
	let totalFiles =
		$(this)[0].files.length + $(galleryElement).find("li").length;
	if (totalFiles > 8) {
