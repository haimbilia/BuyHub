...
<?php if (!empty($images)) { ?>
	<ul id="sortable" class="inline-images">
		...
		foreach ($images as $afile_id => $row) { ?>
			<li id="<?php echo $row['afile_id']; ?>"> <img src="<?php echo UrlHelper::getCachedUrl(UrlHelper::generateFileUrl('image', 'product', array($row['afile_record_id'], "THUMB", 0, $row['afile_id']), CONF_WEBROOT_URL), CONF_IMG_CACHE_TIME, '.jpg'); ?>" title="<?php echo $row['afile_name']; ?>" alt="<?php echo $row['afile_name']; ?>">
