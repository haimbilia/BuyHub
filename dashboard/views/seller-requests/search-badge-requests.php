        ...
        foreach ($arr_flds as $key => $val) {
            ...
            switch ($key) {
                ...
                case 'badge_name':
                    ...
                    $td->appendElement('img', ['src' => UrlHelper::getCachedUrl(UrlHelper::generateFileUrl('Image', 'badgeIcon', array($icon['afile_record_id'], $icon['afile_lang_id'], ImageDimension::VIEW_THUMB, $icon['afile_screen']), CONF_WEBROOT_FRONTEND) . $uploadedTime, CONF_IMG_CACHE_TIME, '.jpg'), 'title' => $name, 'alt' => $name], '', true);
                    break;
                ...
                case 'action':
                    if ($row['breq_status'] == BadgeRequest::REQUEST_PENDING) {
                        ...
                        $li->appendElement(
                            'a',
                            array('href' => 'javascript:void(0)', 'onclick' => "addBadgeReqForm(" . $row['breq_id'] . ", " . $row['badge_id'] . ")", 'class' => 'btn btn-outline-gray btn-sm ', 'title' => Labels::getLabel('LBL_Edit', $siteLangId)),
                            '<i class="icn">
                        ...
                        $li->appendElement(
                            'a',
                            array('href' => 'javascript:void(0)', 'onclick' => "deleteBadgeRequest(" . $row['breq_id'] . ")", 'class' => 'btn btn-outline-gray btn-sm ', 'title' => Labels::getLabel('LBL_DELETE', $siteLangId)),
                            '<i class="icn">
                            <svg class="svg" width="18" height="18">
                                <use
