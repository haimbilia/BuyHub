...
if (!empty($image) && isset($image['afile_id']) && $image['afile_id'] != -1) {
 ...
 echo HtmlHelper::getfileInputHtml(
    ['onChange' => 'brandPopupImage(this)', 'accept' => 'image/*', 'data-name' => $file_type == 'logo' ? Labels::getLabel("FRM_BRAND_LOGO", $siteLangId) : Labels::getLabel("FRM_BRAND_BANNER_IMAGE", $siteLangId)],
    $siteLangId,
    ($canEdit ? 'removeBrandMedia(' . $image['afile_record_id'] . ',\'' . $file_type . '\',' . $image['afile_id'] .',' . $image['afile_lang_id'].',' . $image['afile_screen'] .')' :''),
