...
foreach ($arrListing as $sn => $row) {
    ...
    foreach ($fields as $key => $val) {
        ...
        switch ($key) {
            ...
            case 'product_name':
                $txt = '<div class="product-profile__description">';
                $txt .= '<div class="product-profile__title">' . $row['product_name'] . '</div>';
                if ($row['op_selprod_title'] != '') {
                    $txt .= '<div class="product-profile__sub_title"><strong>' . Labels::getLabel('LBL_Custom_Title', $siteLangId) . ": </strong>" . $row['op_selprod_title'] . '</div>';
                }

                if ($row['op_selprod_options'] != '') {
                    $txt .= '<div class="product-profile__specification">' . Labels::getLabel('LBL_Options', $siteLangId) . ": </strong>" . $row['op_selprod_options'] . '</div>';
                }
                $txt .= '</div>';
