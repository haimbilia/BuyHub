            ...
            switch ($key) {
                case 'promotion_image':
                    if ($row['promotion_type'] == Promotions::PROMOTE_PRODUCT) {
                        $td->appendElement('plaintext', array(), '<div class="avtar"><img src="' . UrlHelper::getCachedUrl(UrlHelper::generateFileUrl('image', 'product', array($product['promotion_product_id'], 'MINI', 0, 0, $siteLangId), CONF_WEBROOT_FRONTEND), CONF_IMG_CACHE_TIME, '.jpg') . '" alt="' . $row["prod_name"] . '"></div>', true);
                    } elseif ($row['promotion_type'] == Promotions::PROMOTE_SHOP) {
                        $td->appendElement('plaintext', array(), '<div class="avtar"><img src="' . UrlHelper::generateFileUrl('image', 'shop', array($product['promotion_shop_id'], 'MINI', 0, 0, $siteLangId), CONF_WEBROOT_FRONTEND) . '" alt="' . $row["shop_identifier"] . '"></div>', true);
