...
(function () {
  ...
  viewWishListItems = function (uwlist_id, append) {
    ...
    $(dv).html(fcom.getLoader());
    fcom.ajax(
      fcom.makeUrl("Account", "viewWishListItems"),
      "uwlist_id=" + uwlist_id,
      function (ans) {
        if (append == 1) {
  ...
  viewFavouriteItems = function (append) {
    ...
    $(dv).html(fcom.getLoader());
    fcom.ajax(
      fcom.makeUrl("Account", "viewFavouriteItems"),
      "",
      function (ans) {
        if (append == 1) {
          $(dv).find(".loader-yk").remove();
  ...
  searchFavouriteListItems = function (frm, append) {
    ...
    fcom.updateWithAjax(
      fcom.makeUrl("Account", "searchFavouriteListItems"),
      data,
      function (ans) {
        $.mbsmessage.close();
        if (append == 1) {
  ...
  searchWishListItems = function (uwlist_id, append, page) {
    ...
    fcom.updateWithAjax(
      fcom.makeUrl("Account", "searchWishListItems"),
      "uwlist_id=" + uwlist_id + "&page=" + page,
      function (ans) {
        $.mbsmessage.close();
        $(dv).find(".loader-yk").remove();
        if (append == 1) {
