...
class EasyPostSettingsController extends ShippingServicesSettingsController
{
    public static function form(int $langId)
    {
        ...
        $frm->addTextBox(Labels::getLabel('FRM_API_KEY', $langId), 'api_key');
        $fld1 = new FormFieldRequirement('api_key', Labels::getLabel('FRM_API_KEY', $langId));
        $fld1->setRequired(false);
        $reqFld1 = new FormFieldRequirement('api_key', Labels::getLabel('FRM_API_KEY', $langId));
        $reqFld1->setRequired(true);


        $frm->addTextBox(Labels::getLabel('FRM_API_KEY', $langId), 'live_api_key');
        $liveFld1 = new FormFieldRequirement('live_api_key', Labels::getLabel('FRM_API_KEY', $langId));
        $liveFld1->setRequired(false);
        $reqLiveFld1 = new FormFieldRequirement('live_api_key', Labels::getLabel('FRM_API_KEY', $langId));
