    ...
    private function getRecommendedProducts($selprod_id, $langId, $userId = 0)
    {
        ...
        $srch->addMultipleFields(
            array(
                ...
                'IF(selprod_stock > 0, 1, 0) AS in_stock', 'selprod_sold_count', 'selprod_return_policy', 'shop_id', 'selprod_cart_type'
            )
        );
    ...
    private function getRecentlyViewedProductsDetail($cookiesProductsArr = array())
    {
        ...
        $prodSrch->addMultipleFields(
            array(
                ...
                'special_price_found', 'splprice_display_list_price', 'splprice_display_dis_val', 'splprice_display_dis_type', 'selprod_sold_count', 'COALESCE(selprod_title, product_name, product_identifier) as selprod_title', 'selprod_price', 'shop_id', 'selprod_cart_type'
            )
        );
        $productRs = $prodSrch->getResultSet();
        ...
        if (isset($ids) && is_array($ids) && count($ids)) {
            ...
            $prodSrch->addMultipleFields(
                array(
                    ...
                    'selprod_id', 'selprod_condition', 'IF(selprod_stock > 0, 1, 0) AS in_stock', 'theprice',
                    'special_price_found', 'splprice_display_list_price', 'splprice_display_dis_val', 'splprice_display_dis_type', 'selprod_sold_count', 'selprod_price', 'selprod_stock', 'selprod_min_order_qty', 'selprod_cart_type'
                )
            );
    ...
    private function getListingData($get)
    {
        ...
        if (FatApp::getConfig('CONF_DEFAULT_PLUGIN_' . Plugin::TYPE_FULL_TEXT_SEARCH, FatUtility::VAR_INT, 0)) {
            ...
            $flds = array(
                ...
                'shop.shop_id', 'shop.shop_lat', 'shop.shop_lng', 'COALESCE(shop_name, shop_identifier) as shop_name', 'selprod_cart_type'
            );
