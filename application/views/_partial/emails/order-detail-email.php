                        ...
                        <tr>
                            <td style="background-color: #' . FatApp::getConfig('CONF_EMAIL_TEMPLATE_COLOR_CODE' . $siteLangId, FatUtility::VAR_STRING, 'ff3a59') . ';padding: 10px 25px;">
                                <table width="100%" border="0" cellpadding="0" cellspacing="0">                                                             
                                    <tr>
                                        <td style="font-size: 14px;font-weight: $font-weight-boldest;color: #333;">' . Labels::getLabel('Lbl_Order_No.', $siteLangId) . ' ' . $orderInfo['order_number'] . '</td>
                                        <td style="font-size: 14px;font-weight: $font-weight-boldest;color: #333; text-align: right;">' . Labels::getLabel('Lbl_Order_Date.', $siteLangId) . ' ' . FatDate::format($orderInfo['order_date_added']) . '</td>
                                    </tr>
                                </table>                                                          
                            </td>
                        </tr> 
...
foreach ($orderProductsData as $addrKey => $orderProducts) {
    ...
    if (!empty($orderProducts['pickupAddress'])) {
        ...
        if ($orderProducts['pickupAddress']['oua_phone'] != '') {
            $pickUpAddressInfo .= ', <span class="default-ltr">' . ValidateElement::formatDialCode($orderProducts['pickupAddress']['oua_phone_dcode']) . $orderProducts['pickupAddress']['oua_phone'] . '</span>';
        }
...
if (!empty($shippingAddress)) {
    ...
    if ($shippingAddress['oua_phone'] != '') {
        $shippingInfo .= '<br><span class="default-ltr">' . ValidateElement::formatDialCode($shippingAddress['oua_phone_dcode']) . $shippingAddress['oua_phone'] . '</span>';
    }
}
