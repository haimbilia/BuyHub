...
<ul class="grouping grouping-level" id="level-link-<?php echo $linkId; ?>">
    <?php foreach ($children as $child) {
        ...
        ?>
        <li class="grouping-item groupingItemJs qeweeqwew">
            <span class="grouping-section groupingSectionJs">
                ...
                <?php if (0 < count($child['children'])) { ?>
                    <button class="grouping-arrow dropdown-toggle-custom collapseBtnJs collapsed" type="button"
                        ...
                        data-bs-target="#navItem-<?php echo $child['nlink_id']; ?>"
                        aria-controls="navItem-<?php echo $child['nlink_id']; ?>">
                        <i class="grouping-arrow-icon dropdown-toggle-custom-arrow"></i>
                    </button>
                <?php } ?>
            </span>
            <?php if (0 < count($child['children'])) { ?>
                <div class="collapse collapseJs" id="navItem-<?php echo $child['nlink_id']; ?>"
                    data-bs-parent="#level-link-<?php echo $linkId; ?>">
