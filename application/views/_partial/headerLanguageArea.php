            ...
            <a class="dropdown-toggle no-after" data-bs-toggle="dropdown" href="javascript:void(0)">
                <?php if ($hasMultipleLangs) {
                    if ($languages[$siteLangId]['language_country_code']) { ?>
                        <img class="icon--img" alt="<?php echo Labels::getLabel('LBL_Language_Flag', $siteLangId); ?>" src="<?php echo CONF_WEBROOT_URL; ?>images/flags/<?php echo FatApp::getConfig('CONF_COUNTRY_FLAG_TYPE', FatUtility::VAR_STRING, 'round'); ?>/<?php echo $languages[$siteLangId]['language_country_code'] . '.svg'; ?>">
                    <?php } ?>
                    ...
                    <?php if ($hasMultipleLangs) { ?>
                        ...
                        <ul>
                            <?php foreach ($languages as $langId => $language) { ?>
                                <li class="<?php echo ($siteLangId == $langId) ? ' is-active' : ''; ?>"><a class="" href="javascript:void(0);" onClick="setSiteDefaultLang(<?php echo $langId; ?>)"><?php if ($language['language_country_code']) { ?>
                        ...
                        <ul>
                            <?php foreach ($currencies as $currencyId => $currency) { ?>
                                <li class="<?php echo ($siteCurrencyId == $currencyId) ? ' is-active' : ''; ?>">
                                    <a class="dropdown-item nav__link" href="javascript:void(0);" onClick="setSiteDefaultCurrency(<?php echo $currencyId; ?>)"> <?php echo $currency; ?></a>
                                </li>
                            <?php } ?>
                        </ul>
                    <?php } ?>
