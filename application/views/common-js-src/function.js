    ...
    if ($(window).width() < 576) {
        ...
        $(".js-footer-group-head").on("click", function () {
            if ($(this).hasClass("is-active")) {
                ...
                $(this).siblings(".js-footer-nav").slideUp();
                return false;
            }
...
function googleCaptcha(updateToken = false) {
    ...
    if ("" != inputObj.val() && false === updateToken) {
        return;
    }
...
function googleAddressAutocomplete(
    ...
    saveCookie = true,
    callback = "googleSelectedAddress"
) {
...
function initMutipleMapMarker(
    ...
    if (typeof dragendCallback == "function") {
        ...
        spanTag.appendChild(
            document.createTextNode(langLbl.searchAsIMoveTheMap)
        );
    ...
    HTMLMarker.prototype.onAdd = function () {
        ...
        this.div.className =
            "float-price " + (this.isDefault == 1 ? "float-brand" : "");
...
function openMobileMenu() {
    if ("" == sidebarHtml) {
        fcom.ajax(
            fcom.makeUrl("Category", "sidebarCategoriesList"),
            "",
            function (res) {
...
function requestForQuoteFn(selprodId) {
    ...
    fcom.ajax(
        fcom.makeUrl("RequestForQuotes", "form"),
        "selprodId=" + selprodId + "&rfqQuat=" + rfqQuat,
        function (res) {
            fcom.closeProcessing();
            if (!res.status) {
                if (
                    typeof res.displayLoginForm != "undefined" &&
                    res.displayLoginForm == 1
                ) {
                    loginPopUpBox(true);
                    return;
                }
...
function saveRfq(frm) {
    ...
    frm.find(
        'select,input[type=hidden],input[type=text],input[type=search], input[type="radio"],textarea'
    ).each(function () {
        ...
        if (typeof isMultiple !== "undefined" && isMultiple !== false) {
            ...
            if (!data.has(eleName)) {
                data.append(
                    eleName,
                    $("input[name='" + eleName + "']:checked").val()
                );
            }
        } else {
            data.append(eleName, $(this).val());
...
function saveMultipleRfq(frm) {
    ...
    $("#sideQuoteJs .rfqBodyJs")
        .addClass("processing-wrap")
        .prepend(fcom.getLoader());
    var data = new FormData();
    data.append("fIsAjax", 1);
    data.append("fOutMode", "json");
    frm.find("select,input[type=hidden],input[type=text],textarea").each(
        function () {
    ...
    $.ajax({
        ...
        success: function (t) {
            ...
            if (t.status == 0) {
                if (
                    typeof t.displayLoginForm != "undefined" &&
                    t.displayLoginForm == 1
                ) {
...
function addAddress(selprodId = 0) {
    ...
    fcom.ajax(
        fcom.makeUrl("RequestForQuotes", "addAddress"),
...
function saveAddress(frm, selprodId, rfq = false) {
    ...
    if (rfq) {
        ...
        data +=
            "&isDefault=" +
            (confirm(langLbl.confirmDefault) ? 1 : 0) +
            "&getHtml=1";
    }
    fcom.updateWithAjax(
        fcom.makeUrl(contName, "setUpAddress", [], webroot),
        data,
        function (ans) {
            fcom.removeLoader();
...
$(document).ready(function () {
    ...
    $(window).click(function (event) {
        ...
        if (
            !target.closest(".addressMenuJs").length &&
            $(".addressListingJs").is(":visible")
        ) {
            $(".addressListingJs").fadeOut();
    ...
    $(document).on("click", ".addressItemJs", function () {
        ...
        sibling
            .find(".btnDropdownContentJs")
            .html($(this).find(".addressItemContentJs").html());
        if (sibling.find(".custom-menu-dropdown-data").length) {
            sibling
                .find(".custom-menu-dropdown-data")
                .removeClass("custom-menu-dropdown-data")
                .addClass("custom-menu-content");
        }
        $(".addressListingJs").fadeOut();
...
$(function () {
    $(document).on("change", ".rfqDocumentJs", function () {
        ...
        nameEle.html(
            `<li class="uploaded-list-item">
              <div class="title">
                ${$(this).val()} 
              </div>
              <button type="button" class="btn-close text-reset resetAttachmentJs"></button>
            </li>`
        );
    });
