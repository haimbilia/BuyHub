...
<?php if (!empty($collectionData)) { ?>
    ...
    <?php foreach ($collectionData as $collection) { ?>
                        <div>
                            <figure class="item"><img class=""
                                                      src="<?php echo UrlHelper::generateFileUrl('Image', 'shopCollectionImage', array($collection['scollection_id'], $siteLangId, 'SHOP')); ?>"
                                                      alt="" data-ratio="2:1"></figure>
                            <div class="overlay-content">
                                <h4><?php echo $collection['scollection_name']; ?></h4>
                                <!--<p>From the runway to your wardrobe</p>-->
                                <a href="<?php echo UrlHelper::generateUrl('Shops', 'collection', array($shop['shop_id'], $collection['scollection_id'])) ?>"
                                   class="link"><?php echo Labels::getLabel('MSG_Explore', $siteLangId) ?></a>
                            </div>
                        </div>
    <?php } ?>
                </div>
