...
$arr_flds = array(
    ...
    Badge::DB_TBL_PREFIX . 'active' => Labels::getLabel('LBL_PUBLISH', $siteLangId),
    'action' => '',
);
    ...
    foreach ($arr_flds as $key => $val) {
        ...
        switch ($key) {
            ...
            case Badge::DB_TBL_PREFIX . 'required_approval':
                ...
                $htm = ' <span class="badge badge--unified-success badge--inline badge--pill">' . Labels::getLabel('LBL_NOT_REQUIRED', $siteLangId) . '</span>';;
                if (Badge::TYPE_BADGE == $row[Badge::DB_TBL_PREFIX . 'type']) {
            ...
            case 'action':
                if ($canEdit) {
                    ...
                    $td->appendElement('a', array('href' => 'javascript:void(0)', 'class' => 'btn btn-clean btn-sm btn-icon', 'title' => Labels::getLabel('LBL_DELETE', $siteLangId), "onclick" => "deleteRecord(event, " . $row[Badge::DB_TBL_PREFIX . 'id'] . ")"), "<i class='fas fa-trash icon'></i>", true);
                } else {
