...
(function () {
	...
	bindBadgeRibbonNameSelect2 = function (type) {
		if (TYPE_BADGE == type) {
			var selector = $("select[name='badge_name']");
			var recordSelector = "input[name='badge_id']";
		} else {
			var selector = $("select[name='ribbon_name']");
		...
		selector.select2({
			...
			ajax: {
				url: function () {
					return fcom.makeUrl('Seller', 'badgeAutocomplete', [type, RECORD_TYPE_SELLER_PRODUCT]);
				},
				dataType: 'json',
				delay: 250,
				method: 'post',
...
$(document).ready(function () {
	bindBadgeRibbonNameSelect2(TYPE_BADGE);
	bindBadgeRibbonNameSelect2(TYPE_RIBBON);
});