...
class OrderSearch extends SearchBase
{
    ...
    public function __construct($langId = 0)
    {
        ...
        if ($this->langId > 0) {
            $this->joinTable(
                Orders::DB_TBL_LANG,
                'LEFT OUTER JOIN',
                'orderlang_order_id = o.order_number AND orderlang_lang_id = ' . $this->langId, 'o_l'
            );
        }
    }
    ...
    {
        $this->joinTable(OrderSubscription::DB_TBL, 'LEFT OUTER JOIN', 'oss.ossubs_order_id = o.order_number', 'oss');
        if ($langId > 0) {
    ...
    public function joinOrderProduct($langId = 0)
    {
        ...
        $this->joinTable(Orders::DB_TBL_ORDER_PRODUCTS, 'LEFT OUTER JOIN', 'op.op_order_id = o.order_number', 'op');
        if ($langId > 0) {
