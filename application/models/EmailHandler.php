...
class EmailHandler extends FatModel
{
    ...
    public function sendForgotPasswordLinkEmail($langId, $d)
    {
        ...
        $phone = !empty($d['user_phone']) ? $d['user_phone'] : '';
    ...
    public function sendResetPasswordConfirmationEmail($langId, &$d)
    {
        ...
        $phone = array_key_exists('user_phone', $d) ? $d['user_phone']  : '';
        $this->sendSms($tpl, $phone, $vars, $langId);
        return true;
    }
    ...
    public function sendCatalogRequestStatusChangeNotification($langId, $d)
    {
        ...
        $phoneNumbers[] = !empty($d['user_phone']) ? $d['user_phone'] : '';
    ...
    public function sendBrandRequestStatusChangeNotification($langId, $data)
    {
        ...
        $userInfo = $userObj->getSellerData($langId, array('user_id', 'ifnull(shop_name, shop_identifier) as shop_name', 'user_phone', 'credential_email'));
    ...
    public function sendCustomCatalogRequestStatusChangeNotification($langId, $d)
    {
        ...
        $phoneNumbers = $receipentsInfo['phone'];
        $userPhone = !empty($d['user_phone']) ? $d['user_phone'] : '';
        $phoneNumbers[] = $userPhone;
        foreach ($phoneNumbers as $phone) {
            $this->sendSms($tpl, $phone, $vars, $langId);
        }
    ...
    public function newOrderBuyerAdmin($order_id, $langId = 0, $includeAdmin = true, $pushNotification = true)
    {
        ...
        if ($orderInfo) {
            ...
            $userInfo = $userObj->getUserInfo(array('user_name', 'credential_email', 'user_phone'), true, false);
    ...
    public function sendProductStockAlert($selprod_id, $langId = 0)
    {
        ...
        $srch->addMultipleFields(array('selprod_title', 'selprod_product_id', 'user_id', 'user_name', 'user_phone', 'credential_email', 'ifnull(shop_name, shop_identifier) as shop_name'));
        $srch->doNotCalculateRecords();
        $srch->doNotLimitRecords();
        $rs = $srch->getResultSet();
        ...
        $phoneNumbers = $receipentsInfo['phone'];
        $userPhone = !empty($productInfo['user_phone']) ? $productInfo['user_phone'] : '';
        $phoneNumbers[] = $userPhone;
        foreach ($phoneNumbers as $phone) {
        ...
        if ($orderDetail) {
            ...
            foreach ($orderVendors as $key => $val) :
                ...
                if (!in_array(strtolower($paymentType), ['cashondelivery', 'payatstore', 'transferbank'])) {
                    ...
                    $sellerInfo = User::getAttributesById($val['op_selprod_user_id'], array('user_phone'));
                    $sellerPhone = !empty($sellerInfo['user_phone']) ? $sellerInfo['user_phone'] : '';
    ...
    public function sendTxnNotification($txnId, $langId)
    {
        ...
        $txnDetail = $txn->getAttributesWithUserInfo(0, array('utxn_credit', 'utxn_debit', 'utxn_comments', 'user_name', 'user_phone', 'credential_email', 'utxn_user_id'));
        ...
        $phone = !empty($txnDetail['user_phone']) ? $txnDetail['user_phone'] : '';
    ...
    public function sendWithdrawRequestNotification($requestId, $langId, $adminOrUser = "A")
    {
        ...
        $srch->addMultipleFields(
            array(
                'tuwr.*', 'GROUP_CONCAT(CONCAT(`uwrs_key`, ":", `uwrs_value`)) as payout_detail', 'user_name', 'credential_email as user_email', 'credential_username as user_username', 'user_phone'
            )
        );
        ...
        if ($adminOrUser == "A") {
            ...
            $phone = !empty($withdrawalRequestData['user_phone']) ? $withdrawalRequestData['user_phone'] : '';
        }
    ...
    public function sendMessageNotification($messageId, $langId)
    {
        ...
        $uData = User::getAttributesById($message["message_to"], ['user_phone']);
        if (!empty($uData)) {
    ...
    public function sendOrderReturnRequestStatusChangeNotification($orrequest_id, $langId)
    {
        ...
        $msgSrch->addMultipleFields(
            array(
                'orrmsg_id', 'orrmsg_from_user_id', 'user_name', 'user_phone',
                'orrmsg_from_admin_id', 'admin_name', 'admin_username'
            )
        );
        ...
        $phone = !empty($lastMsgRow['user_phone']) ? $lastMsgRow['user_phone'] : '';
    ...
    public function sendOrderCancellationRequestUpdateNotification($ocrequest_id, $langId)
    {
        ...
        $srch->addMultipleFields(
            array(
                ...
                'op_invoice_number', 'buyer.user_name as buyer_name', 'buyer.user_phone as buyer_phone', 'buyer_cred.credential_email as buyer_email', 'buyer.user_id as buyer_id'
            )
        );
        $rs = $srch->getResultSet();
    ...
    public function sendBlogCommentStatusChangeEmail($langId, $d)
    {
        ...
        $uData = User::getAttributesById($d['bpcomment_user_id'], ['user_phone']);
    ...
    public function sendBuyerReviewStatusUpdatedNotification($spreviewId, $langId = 0)
    {
        ...
        $schObj->addMultipleFields(array('spreview_selprod_id', 'spreview_status', 'IFNULL(selprod_title  ,IFNULL(product_name, product_identifier)) as selprod_title', 'user_name', 'user_phone', 'credential_email'));
        ...
        $phone = !empty($spreviewData['user_phone']) ? $spreviewData['user_phone'] : '';
    ...
    public function sendAffiliateMailShare($senderId, $receiverEmail, $personalMsg, $langId)
    {
        ...
        $srch->addMultipleFields(array('u.user_phone'));
